...
var shoppingCart = (function () {
  ...
  obj.removeItemFromCart = function (productcode) {
    // Removes one item
    for (var i in cart) {
      if (cart[i].productcode === productcode) {
        // "3" === 3 false
  ...
  obj.removeItemFromCartAll = function (productcode) {
  ...
  if (cartArray.length <= 0) {
    ...
    for (var i in cartArray) {
      output +=
        ...
        "<div class='bt_remove_product left' data-product-code='" +
        cartArray[i].productcode +
        "'>" +
        "</div>" +
        "</td>" +
        ...
        "<input class='bt_minus' type='button' value='-' data-product-code='" +
        cartArray[i].productcode +
        "'>" +
        "<input type='text' disabled='' name='product_cart_qty' value='" +
        cartArray[i].count +
        "' title='Qty' class='input-text qty text product_qty' size='4'>" +
        "<input class='bt_plus' type='button' value='+' data-product-code='" +
        cartArray[i].productcode +
        "'>" +
        "</div>" +
        "</div>" +
        ...
        "<span class='amount'>" +
        numberWithCommas(cartArray[i].total) +
        "</span>" +
        "</td>" +
        "</tr>";
    }
...
function displayCartPageCart() {
  ...
  if (cartArray.length <= 0) {
    output =
      "<tr><td colspan='7' style='text-align: center'><p style='padding-left: 20px;'>Chưa có sản phẩm nào</p></td></tr>";
  } else {
    for (var i in cartArray) {
      output +=
        "<tr>" +
        "<td class='cart_product'>" +
        "<a href='#'><img src='images/product-detail/" +
        cartArray[i].image +
        "' alt='Product'></a>" +
        "</td>" +
        ...
        "<td class='price'><span style='color: #e84d1c; font-size: 18px; font-weight: bold;'>" +
        numberWithCommas(cartArray[i].price) +
        "</span></td>" +
        "<td class='qty'>" +
        "<input class='form-control input-sm' type='text' readonly value='" +
        cartArray[i].count +
        "'>" +
        "<a class='plus' data-product-code='" +
        cartArray[i].productcode +
        "'><i class='fa fa-caret-up'></i></a>" +
        "<a class='minus' data-product-code='" +
        cartArray[i].productcode +
        "'><i class='fa fa-caret-down'></i></a>" +
        "</td>" +
        "<td class='price'>" +
        "<span style='color: #e84d1c; font-size: 18px; font-weight: bold;'>" +
        numberWithCommas(cartArray[i].total) +
        "</span>" +
        "</td>" +
        "<td class='action'>" +
        "<a class='remove_product' data-product-code='" +
        cartArray[i].productcode +
        "'>Delete item</a>" +
        "</td>" +
        "</tr>";
