...
export class TinTucComponent {
    ...
    edit(tintuc: ITinTuc) {
        this.tintucService.getById(tintuc.id).subscribe((data) => {
            ...
            this.fileOnly = { name: data.duongDanAnh } as File;
            this.Dialog = true;
    ...
    save() {
        this.tintuc.loaiTinId = this.selectedLoaiId.id;
        if (this.fileOnly) {
            this.tintuc.duongDanAnh = this.fileOnly.name;
        }
        this.tintuc.nguoiVietId = Number(this.authen.getIdFromToken());
        this.submitted = true;
        if (this.fileOne) {
            this.onUpload();
            this.fileOne = false;
